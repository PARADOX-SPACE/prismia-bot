...
async def monitor_status_ss14():
    ...
    """
    # Здесь будет код для получения статуса сервера и отправки уведомлений при изменении
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as resp:
                if resp.status == 200:
                    ...
                    bot.change_presence(activity=Game(name=f"{data['players']}/{data['soft_max_players']} на {data['map']}"))
                else:
